/*
 * @Author: Divenire
 * @Date: 2021-10-18 10:46:54
 * @LastEditors: Divenire
 * @LastEditTime: 2021-10-19 17:15:23
 * @Description: 
 *              extract ORB cost = 0.0195896 seconds. 
                match ORB cost = 0.0296367 seconds. 
                bad/total: 43/638 左图43个边缘点，未参与描述子计算
                bad/total: 8/595 右图8个边缘点，未参与描述子计算
                matches: 41 匹配点
                可以看到600个点只有41个匹配上了- -效果不好
 */
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
  ...
  for (auto &kp : keypoints)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      {
        int idx_pq = i * 32 + k;
        //利用ORB Parrten 生成的一对坐标
        cv::Point2f p(ORB_pattern[idx_pq * 4], ORB_pattern[idx_pq * 4 + 1]);
        cv::Point2f q(ORB_pattern[idx_pq * 4 + 2], ORB_pattern[idx_pq * 4 + 3]);

        //利用旋转不变性， rotate with theta，保证图像在不同角度旋转后得到的坐标是同一个位置
        ...
        // 利用两个坐标的灰度值大小作为相对关系，如果pp小于qq，那么这位就置1.
        //一共是32*8位。
        //相应的ORB parttern有256组。
        if (img.at<uchar>(pp.y, pp.x) < img.at<uchar>(qq.y, qq.x))
        {
          d |= 1 << k;
        }
      }
      // 每次存储32位比较结果，也就是描述子的一部分
      desc[i] = d;
    }
